...
import { Send, Plus, Settings, User, Sparkles, X, Maximize2, Minimize2, ChevronDown } from "lucide-react";
...
interface FuturisticAIChatbotProps {
  chatId?: number; // Optional prop for future chat selection
}
...
const Message = ({ isUser, content, timestamp }: { isUser: boolean; content: string; timestamp: string }) => {
  return (
...
export default function FuturisticAIChatbot({ chatId }: FuturisticAIChatbotProps) {
      ...
      <div className="flex-1 overflow-y-auto py-4 px-4 z-10">
        ...
        <div className="space-y-4">
          {messages.map((message) => (
            <Message key={message.id} isUser={message.isUser} content={message.content} timestamp={message.timestamp} />
          ))}
